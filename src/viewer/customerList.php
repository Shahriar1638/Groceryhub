      ...
      <section class="pl-40 pt-4 h-screen">
        <div class="grid grid-cols-6">
          ...
          <div class="col-span-5 bg-white rounded-tl-3xl h-screen pl-12 pt-12">
            <div>
              <h1 class="text-4xl font-bold uppercase text-center mb-8">Customer List</h1>
            </div>
            <div class='overflow-x-auto mt-8'>
                <table class='table'>
                    <thead>
                        <tr>
                            <th class="uppercase">Customer Name</th>
                        ...
                        if (mysqli_num_rows($result) > 0){
                            while ($row = mysqli_fetch_assoc($result)){
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($banStatus == '0') { ?>
                                            <button class="px-6 py-2 rounded-md bg-green-500 uppercase text-white" onclick="handleBanStatus('<?php echo $customerEmail ?>', 'unban','customer')">Unban</button>
                                        <?php } else { ?>
                                            <button class="px-6 py-2 rounded-md bg-red-500 uppercase text-white" onclick="handleBanStatus('<?php echo $customerEmail ?>', 'ban', 'customer')">Ban</button>
                                        <?php } ?>
                                    </td>
                                </tr>
                        <?php
